...
func (c *Client) Call(name string, fptr interface{}) {
	...
	f := func(req []reflect.Value) []reflect.Value {
		cliTransport := NewSession(c.conn)
		...
		err := cliTransport.SendMsg(RpcMessage{FuncName: name, Args: inArgs})
		if err != nil { // local network error or encode error
			return errorHandler(err)
		}
		// receive response from server
		rsp, err := cliTransport.RecvMsg()
		if err != nil { // local network error or decode error
			return errorHandler(err)
		}
		if rsp.ErrMsg != "" { // remote server error
			return errorHandler(errors.New(rsp.ErrMsg))
		}
